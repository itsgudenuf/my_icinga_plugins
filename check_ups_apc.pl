...
#
# 2023-12-23: Brian (at) walters.guru
#   - added on battery crit and warn
...
# crit / warn values
$remaining_time_crit = 15;
$remaining_time_warn = 30;
...
sub main {
	...
	given ( $minutes ) {
          when( $_ >= $on_battery_crit ) {
                $returnstring = $returnstring . "CRIT  MINUTES ON BATTERY: $minutes - ";
                $status = 2;
          }
          when ( $_ >= $on_battery_warn ) {
                $returnstring = $returnstring . "WARN MINUTES ON BATTERY: $minutes - ";
                $status = 1;
          }
          default {
...
NOTE:  Valus for remaining battery time and on battery time have
       been hard coded for the site
